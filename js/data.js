  ...
  cases: [
    ...
    },
    {
      ...
      story: [
        ...
        { kind: "figure", hue: 150, label: "Limbic Types · user mapping", caption: "Fig. 3 - The Limbic Types segmentation, mapped to our actual user base. Replaced the 2018 personas." },
  ...
  posts: [
    {
      ...
      body: [
        { kind: "lead", text: "A confession to open with: when ChatGPT first showed up at work, I pointed it straight at my research data, both the qualitative and the quantitative side. … after making sure it didn't include any personal data that is! And what happened when I asked it to analyze? It failed at both." },

        { kind: "paragraph", text: "The clearest miss was a pile of open-text entries, thousands of them, that I wanted clustered for quantitative text analysis. What came back looked great and was mostly invented: categories that didn't reflect the content, an importance order it made up, and, the part that made me close the tab, quotes it had written itself and served up as real user verbatim in the reporting draft. I saw the same thing later on interview data. I tried it briefly and dropped it. Same for quantitative data: numbers fully made up (no wonder, it's probabilistic), and even if numbers hold up, the interpretation was a nightmare." },

        { kind: "h2", text: "What actually changed" },
        { kind: "paragraph", text: "It definitely wasn't a smarter model or a cleverer prompt. The situation changed when I started using AI to write code, and let Python run my survey analysis instead of the chatbot. The numbers stopped going through the model's guesswork and started going through a script. (The pipeline itself I've described [in another post](#/blog/better-designer-with-ai), so I'll skip the how here.)" },
        { kind: "paragraph", text: "**The fix wasn't trusting the AI more, it was taking the calculation out of its hands.** Kind of back to basics: before AI existed, Python was a valid way to analyze quant data, so why not bring that back and utilise it." },

        { kind: "h2", text: "Why taking it out of its hands works" },
        { kind: "paragraph", text: "Someone at a research roundtable recently asked me to explain why a Python analysis should be any more trustworthy than asking the model directly. The answer is easy: A language model doesn't calculate, it predicts the next token. Ask it for a mean and it returns the most plausible-looking number, a guess in the shape of an answer. Ask it to write the Python, and a separate interpreter computes that mean deterministically. The model stops being the calculator and becomes the author of a procedure: one I can read, check, and run again." },
        { kind: "paragraph", text: "And here comes the real benefit, and it isn't \"the answer is automatically right.\" Why? Because a reproducible analysis can still be the wrong analysis. What I get is no black box: the script is transparent, it's repeatable, and I can attach it straight to my research report. My data and methods aren't just described, they can be verified and re-run by anyone who doubts them. The arithmetic is the machine's job. But choosing the right test and reading what the result means stays a human job. AI tempts to forget about context (the AI will figure it out anyways, right). You as a researcher and designer know about the context, and it's your job to check in on that." },
        { kind: "paragraph", text: "If I choose a wrong test for my data it won't work. If I don't make sure to clean up my raw data, the results will be flawed. I may not realize it, but this is exactly where human expertise by subject matter experts is needed." },

        { kind: "h2", text: "Why I still keep it away from qual" },
        { kind: "paragraph", text: "Here's the asymmetry that makes me favor AI for quant over qual analysis. In quant there's a ground truth that catches a lie: the counts have to add up to n, the percentages have to total 100, a sum that doesn't close tells me something broke. You can run the calculation to check if a result is correct. In qualitative work that safety net is gone. A made-up theme looks exactly like a real one. No total fails to balance, so the error is invisible, and it lands precisely where the insight was supposed to be." },
        { kind: "paragraph", text: "And it's the failure mode I already watched happen: clusters drawn wrong, their importance inflated, interpretations that were a nightmare to untangle (or didn't make sense at all), and quotes I had to check line by line because some were never said. That isn't a model waiting for a better prompt. It's a task with nothing to check itself against. It gets even worse when you have a democratized research setup. Qual seems easy to do but knowing the methodological backbone of research design, conduction and analysis is the only way to check in with the results an AI can deliver." },
        { kind: "paragraph", text: "I'll be honest about the edge of what I've tested: I haven't built the same code-backed setup for qualitative data, haven't looked into that yet. But I'm skeptical it would rescue it. You can make the mechanical parts reproducible: dedupe, count, cluster on fixed rules. But the interpretive leap, what a theme means and whether a quote actually supports it, has no ground truth to measure against. That's not a tooling gap. It's the nature of qual." },
        { kind: "paragraph", text: "I assume that AI will be well able to do qualitative text analysis of interviews using deductive clustering. But that means it still relies strongly on human input: for study design, analysis framework and finalising the analysis - much more so than quant needs it." },
        { kind: "paragraph", text: "So I split the work by where that ground truth exists. Numbers: the code runs it, I judge it. Meaning: I read the transcripts myself, I keep the participant's own words, and I never hand the synthesis to a machine that would happily invent a tidy one. The day that changes, I'll happily reconsider. Until that I'll keep trying and give AI its fair chance." }
      ]
    },
    {
