  ...
  posts: [
    {
      ...
      body: [
        ...
        { kind: "paragraph", text: "A traditional audit ends as a static artifact, sitting next to a product it can't touch. Each finding is a slide. The reader has to hold the slide and the live page in their head at the same time and **mentally overlay** one on the other. The gap between \"here is what's wrong\" and \"here is the change\" is exactly where momentum gets lost. The findings can sit in that file forever, and in my experience many of them do." },
        { kind: "paragraph", text: "This isn't the first time I pushed back on the slide deck. For years I used [Heurio](https://heurio.co), a tool that lets you annotate and score findings right in the interface. The results land on a Kanban board sorted by priority, and from there you can treat them like a shared to-do list with the devs or export tasks to other tools. I was a regular Heurio user for a long time. Then along came AI." },
        { kind: "paragraph", text: "So I stopped producing artifacts outside the product and started producing inside a copy of it. Everything below follows from that one move." },

        { kind: "h2", text: "A disposable replica" },
        { kind: "paragraph", text: "Read access is all I need: read access to the repo, plus the access Claude Design needs to import it. From there I rebuild the page as a standalone HTML copy, native HTML is Claude Design's default, in a single chat. No write access, no branch, nothing that can touch production. Then I make more copies in that same chat: one to audit, one to redesign, one to keep clean as a reference. They sit side by side, all shareable, none of them touching the real codebase." },
        ...
        { kind: "paragraph", text: "The point isn't to keep the repo tidy, branches already do that. The point is that a copy is something I can share with the client and something they can open without any technical setup. No deploy, no staging URL, no asking a non-technical stakeholder to find their way around GitHub. A link, and they're looking at the real thing. Better still, they can leave comments directly in the interface." },
        ...
        { kind: "paragraph", text: "On the audit copy I prompt Claude to walk the page and drop pins on the issues, one pin per finding, located on the actual UI. It's the thing a tool like Heurio does, except the pins live on a working copy I can immediately edit and carry back into code if I want to. And because I'm building the audit tool here rather than renting it, I can add whatever lens I need. I usually start with Nielsen's heuristics, then enrich them with psychological effects from [Coglode](https://www.coglode.com), a library a lot like the Laws of UX but much broader." },
        { kind: "paragraph", text: "So the set of heuristics is a parameter, not a fixed checklist. I tell Claude which lenses to apply depending on what the client actually needs:" },
        { kind: "list", style: "bullet", items: [
          ...
          { title: "Dark patterns.", text: "The inverse lens: not where the UI fails the user, but where it quietly works against them. Forced continuity, confirmshaming, items sneaked into the basket. Worth a pass before it turns into a trust problem." },
          { title: "Accessibility.", text: "A WCAG-led pass on contrast, focus order, target size, and labels. Often the easiest findings to defend, because the bar is a published standard, not my opinion." }
        ]},
        { kind: "paragraph", text: "For Paperclip.inc that pass surfaced things like a primary CTA competing with three secondary buttons of equal weight, a plan toggle buried below the fold on the pricing page, and a sign-up form with no inline validation. Each one is a pin you can click, on the page it's about." },
        ...
        { kind: "paragraph", text: "I iterate the pins until they're right, drop the ones that don't hold up, and sharpen the wording on the ones that do. This is exactly where AI can't substitute for the human. It does a strong first pass and genuinely enriches the result with findings I'd miss in the sheer mass of lenses you could look at a site through. But it isn't reliable on its own. A [2025 study](https://arxiv.org/abs/2506.16345) comparing GPT-4o to human experts on heuristic evaluation found the model matched only about a fifth of the issues the experts caught, while adding false positives of its own." },
        { kind: "paragraph", text: "That matches what I see. AI finds more than I would alone and far faster, but it over-flags. It'll pin \"low contrast\" on something that's fine, or cite a law that doesn't apply to the case in front of it. The heuristics give it vocabulary, not judgment. I cut maybe a third of what it surfaces before anything reaches the client." },
        { kind: "paragraph", text: "And that's the actual point. The value isn't that the first pass is right. It's that it's cheap enough that me being the filter costs almost nothing. I spend my time deciding which findings are real and which fix is worth building, not making screenshots and formatting slides. I add the ones it missed, and working against its output gets me to ideas I wouldn't have reached alone. The slow, low-judgment part is gone. The part that needs a designer is the only part left." },
        { kind: "figure", hue: 200, label: "Image placeholder", caption: "Fig. 2 - a pinned finding in Claude Design, expanded to show the heuristic, the problem, and the severity." },

        { kind: "h2", text: "Let the client poke at it" },
        { kind: "paragraph", text: "When the audit is done, the more interesting part starts. This is the part a slide deck simply can't do. The audit copy is a real page at a shareable link that looks and behaves exactly like the original. The client opens it, sees the pins in context, and leaves comments directly on the artifact, using Claude Design's built-in commenting. No deploy, no repo, no technical knowledge required on their side. The audit stops being a thing I present and becomes a thing we work on together." },
        ...
        { kind: "paragraph", text: "It gets better. When the client wants the change and not just the diagnosis, I spin up another copy and have Claude build the fixes straight into it. Now there are two live pages at two links: the problem with its pins, and the proposed redesign. Both working, both shareable, both open to the same kind of feedback we're used to in Figma." },
        { kind: "paragraph", text: "This is where **faster** becomes **further**. The audit already contains its own prototype. There's no separate \"and now we'd mock it up\" phase, because the mockup is the next HTML copy in the same chat. The diagnosis and the proposal ship as one artifact, and Claude Design even surfaces the comments so the redesign can build directly on them." },

        { kind: "h2", text: "When it needs to land in the repo" },
        { kind: "paragraph", text: "There's more. The real bottleneck in any audit is the translation step. If you've ever run an audit, or had one run for you, you know you usually end up with an abundance of possible fixes. Someone has to turn those findings into dev tasks, scope them, ticket them, schedule them. That handoff is where the work stalls and the audit goes stale, and where at least some findings never see the light of day." },
        { kind: "paragraph", text: "Beyond turning findings into changes, the code push is the last step. A typical audit stops at the findings, and that's fine. But when the changes are wanted, it's now easy to make them land. I export to Claude Code, which works against the real repo: it makes the edits, commits, and opens a pull request. The finding becomes a commit with no ticket in between, a code-level change a designer has already checked. And if a team prefers tickets, the same handoff can export tasks to Jira and feed the normal sprint. Either way, the translation step stops being the place momentum dies." },
        ...
        { kind: "callout", label: "Why two tools", text: "Claude Design is the client-facing artifact: audit it, pin it, share it, redesign it. Claude Code is the handoff when a change has to enter the repo and survive review. Splitting them isn't a workaround, it's playing each to its strength, and it's what turns \"audit\" into a pipeline. When it fits the client, I can run the whole flow in Claude Code directly. I just find Claude Design friendlier for clients without development experience." },

        { kind: "h2", text: "Final thoughts" },
        { kind: "paragraph", text: "None of this replaces the judgment an audit is actually for. It moves the slow, mechanical part, the screenshots, the formatting, the translation into tickets, out of the way, and leaves me with the part that needs a designer: deciding what's real and what's worth building. The audit used to be a document I handed over. Now it's a working copy the client and I shape together, and when it's wanted, it walks itself into the repo." },
        { kind: "divider", symbol: "◆ ◆ ◆" }
      ]
    },
    {
